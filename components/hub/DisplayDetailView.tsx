...
function LivePreview({ display }: { display: DisplayDetail }) {
  ...
  // A webpage has no thumbnail, so render the live page itself — scaled from
  // the panel's native resolution so this reads as a true miniature of the
  // wall. Only one of these mounts here (a single display), unlike the
  // dashboard wall, where WebpagePreview additionally defers mounting until
  // the tile scrolls into view.
  if (item.type === "WEBPAGE") {
    return (
      <WebpagePreview src={item.fileUrl} orientation={display.orientation} title={item.title} />
    );
  }
